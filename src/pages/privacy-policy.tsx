...
export default function TermsOfServicePage() {
    ...
    const { data, loading } = useCMSAPI("/items/privacy_policy", {
        defaultValue: [],
        skip: 0,
        fields: {
            translations: [
    ...
    const policy = data.translations?.find((item) => item.languages_code === lang);
        ...
        <main className="relative bg-white overflow-auto">
            ...
            <section id="content" className="relative pb-20">
                <div className="container grid gap-28 font-poppins">
                    {!loading && <article className="prose mx-auto lg:max-w-3xl" dangerouslySetInnerHTML={{ __html: policy?.content }} />}
                    {/*  */}
                </div>
            </section>
